...
function getFilteredSkills(charId,skillName){
    let skills = getSkills(charId);
    
    if(skills && skills.length > 0) {
        return skills.filter(s => s.name && s.name.toLowerCase() == skillName.toLowerCase());
    }
    
    return null;
}

function getFilteredFlaws(charId, flawName) {
    let flaws = getFlaws(charId);
    
    if(flaws && flaws.length > 0) {
        return flaws.filter(f => f.name && f.name.toLowerCase() == flawName.toLowerCase());
    }
...
function getFlaws(charId) {
    ...
    rawFlaws.forEach(function(rawFlaw) {
        ...
        } else if(flawName.indexOf('skillnotes') > -1) {
            if(oldFlaw) {
                oldFlaw.notes = rawFlaw.get('current');
            } else {
                flaws.push({ id: flawId, notes: rawFlaw.get('current') });
            }
        }
    });
...
function getTechs(charId) {
    ...
    rawTechs.forEach(function(rawTech) {
        ...
        } else if(techName.indexOf('tech_element') > -1) {
            if(oldTech) {
                oldTech.element = rawTech.get('current');
            } else {
...
function getTechDescription(tech, charId, suppressDamageDisplay) {
    ...
    if(state.elementalSystemOn && tech.element != 'none')
    {
        ...
        {
            summary += '<br /> <small style="text-transform:capitalize">Element: ' + tech.element + '</small>';
        }
    }
...
function getElementalAdjustmentForCharacter(characterId, element)
{
    let adjustment = 0;

    let resistances = getFilteredSkills(characterId,'elementalResistance');
    if(resistances)
    {
        resistances.forEach(function(s){
            if(s.notes.toLowerCase() == element)
            {
                adjustment += (2 + (4 * s.level));
            }
        });
    }

    let vulnerabilities = getFilteredFlaws(characterId,'elementalVulnerability');
    if(vulnerabilities)
    {
        vulnerabilities.forEach(function(s){
            if(s.notes.toLowerCase() == element)
            {
                adjustment -= (2 + (4 * s.level));
            }
...
on('chat:message', function(msg) {
    ...
    msg.content == '!t')) {
        ...
        if(targetsList.length > 0) {
            ...
            let attunement = getSkill(actor.get('_id'),'elementalAttunement');
            let element = 'none';
            if(state.elementalSystemOn)
            {             
                if(attunement)
                {
                    element = attunement.notes.toLowerCase();
                }

                let isElemental = tech.mods && tech.mods.find(function(m) {
                    return m.toLowerCase().indexOf('elemental') > -1;
                });
